...
export default function CartScreen() {
 ...
 return (
   <View className="bg-white flex-1">
    ...
    >
        {/* <Image source={require('../assets/image/bikeGuy.png')} className="w-20 h-20 rounded-full"/> */}
        <Text className="flex-1 pl-4">Deliver in 2-3 hovers</Text>
        <TouchableOpacity>
    ...
    Object.entries(groupedItems).map(([key, items])=>{
        ...
        return (
            ...
            className="flex-row items-center space-x-3 py-2 px-4 bg-white rounded-3xl mx-2 mb-3 shadow-md">
                ...
                <Image className="h-14 w-14 rounded-full" source={camptool.image} />
                <Text className="flex-1 font-bold text-gray-700">{camptool.name}</Text>
                <Text className="font-semibold text-base">Rs{camptool.price}</Text>
                <TouchableOpacity
                onPress={()=> dispatch(removeFromCart({id: camptool.id}))}
